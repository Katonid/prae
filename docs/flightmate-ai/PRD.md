...
- **Scope-Ergänzung (07/2026, dritte Stufe — Reisefestigkeit):** (a) **USA** als vollwertiger Rechtsraum über die offenen FAA-Dienste (UAS Facility Maps/LAANC-Grids mit Höhen-Obergrenze, Luftraumklassen, Special Use Airspace) und die NPS-Parkgrenzen — komplett ohne Schlüssel; New-York-City-Verbot als deterministische Regel; TFRs/State Parks/Stadien als ehrliche Lücke mit B4UFLY-Verweis. (b) **EU-Nachbarländer** (NL, BE, LU, FR, DK, CZ, PL, AT) als gemeinsamer Provider: harmonisierte EU-Basisregeln (Open A1/C0) plus redaktionelle Landes-Besonderheiten, Lufträume über openAIP, nationale Geozonen-Portale je Land als Gegenprüf-Hinweis (keine durchgängig offenen Schnittstellen — ehrliche Lücke). (c) **Nationale Geozonen direkt in der App** (Nutzerfeedback: kein Verweis auf externe Portale), wo amtliche offene Dienste existieren: Niederlande (Natura 2000 via PDOK), Frankreich (DGAC-Restriktionskarte via IGN), Luxemburg (amtliche ED-269-Geozonen). BE/DK/CZ/PL/AT bleiben Portal-Verweis, bis offene Dienste verfügbar sind. (d) **Kanada zweite Stufe** (gleicher Nutzerwunsch): Waldbrand-Sperrzonen live aus den CWFIS-Satelliten-Hotspots von NRCan (9,3-km-Regel, CARs 601.15), Ontario-Provinzparks vom offenen LIO-Dienst der Provinz, kleine Flugplätze/Heliports über openAIP (3-NM-/1-NM-Hinweis) — im Legal-Check und als Karten-Overlays. Verbleibende ehrliche Lücken: NOTAMs, Provinzparks außerhalb Ontarios. (e) **F4 zweite Stufe + Phase-0-Start (07/2026):** Hintergrund-Aktualisierung der Benachrichtigungen per BGAppRefresh (on-device, iOS bestimmt die Laufzeitpunkte; Widget vom Product Owner bewusst abgelehnt) und Score-Validierung als Ein-Tipp-Tagesrückmeldung im Heute-Tab (lokal, 90 Tage, manuelle Nachjustierung der Gewichte statt automatischem Lernen). (f) **Flug-Logbuch (07/2026, Scope-Ergänzung auf Nutzerwunsch):** Ein Eintrag pro Flugtag (Datum, Spot, Score, Ein-Tipp-Bewertung, Notiz, max. 3 verkleinerte Fotos, lokal) im umbenannten „Flüge"-Tab (Logbuch ⇄ KI-Bildkritik). Heutige Einträge mit Bewertung speisen die Phase-0-Score-Kalibrierung. (g) **Geräte-Sync & Datenhoheit (07/2026):** Spots/Drohnenmodell über iCloud-Key-Value-Store, API-Schlüssel über den iCloud-Schlüsselbund (Ende-zu-Ende-verschlüsselt, keine eigene Server-Infrastruktur — Präzisierung zu Kap. 11: Sync bleibt in Apples Nutzer-iCloud, FlightMate betreibt weiterhin keinen Account). JSON-Export/-Import in den Einstellungen (ohne Schlüssel). (h) **Tschechien live + Design-Auffrischung (07/2026):** Das amtliche DronView-Raster der Flugsicherung ŘLP (dronemap.gov.cz) ist inzwischen offen als GeoJSON abrufbar und wird als fünfte nationale Live-Quelle angebunden (Flugverbotszellen und abgesenkte Höhengrenzen je Rasterzelle, lokale Punktprüfung, 7-Tage-Gerätecache, Karten-Overlays) — Präzisierung zu (c): nur noch BE/PL/AT bleiben Portal-Verweis (07/2026 erneut geprüft: Login-/Gateway-gesperrt). Gestaltung: Heute-Tab mit Himmelsverlauf nach echtem Sonnenstand (deterministisch aus den Sonnenzeiten, kein Zufall), Tageszeit-Gruß, federnd aufbauendem Score-Ring mit weichem Schein und app-weitem Milchglas-Kartenstil — Farbe bleibt Ergänzung zu Zahl und Text (Kap.-10-Grundsatz unverändert). (i) **Logbuch-Ausbau nach erstem Feldtest (07/2026):** Jeder Logbuch-Eintrag trägt einen Flugort (beim Anlegen automatisch der Standort, nachträglich über eine zoombare Karte änderbar; Mini-Karte im Eintrag, Antippen vergrößert). Geloggte Flüge erscheinen als lila Marker fest auf der Zonenkarte, je Eintrag abschaltbar. Die Einträge synchronisieren (ohne Fotos) über den iCloud-Key-Value-Store — Ergänzung zu (g), die Fotos bleiben bewusst lokal. Einträge lassen sich einzeln oder im Block löschen und als GPX sichern (offener Wegpunkt-Standard als bewusst gewähltes, langfristig lesbares Format). (j) **Drone Media Explorer (07/2026, in Abstimmung):** Ausbau zum persönlichen Flug-, Foto- und Reisearchiv (Medienimport aus DJI Fly/Apple Fotos/SD/Ordnern, vollständige EXIF-/Video-Metadaten, Standort-Kaskade mit Quelle+Konfidenz, Flugrouten, Reise-/Spot-Erkennung, bearbeitete Versionen, lokale KI-Suche, Katalog-Sync über CloudKit). Grundprinzip: Originale werden nie verändert; alles lebt im Katalog. Vollständiges Konzept: `docs/flightmate-ai/MEDIA-EXPLORER-ARCHITEKTUR.md` — Implementierung erst nach Abnahme des Dokuments.
